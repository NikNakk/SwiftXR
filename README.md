...
## Current milestone: runtime probe

The first executable, `swiftxr-probe`, only proves the lowest layer:
...
The OpenXR development headers and loader library (`libopenxr_loader`) must be available to the compiler/linker when building and running the probe.
...
6. `swiftxr-hello`: render a minimal stereo scene and prove head-tracked presentation through an OpenXR runtime on macOS.
