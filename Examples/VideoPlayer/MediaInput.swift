...
enum MediaInputError: Error, CustomStringConvertible {
    ...
    var description: String {
        ...
        case .noDownloadedFile:
            return "yt-dlp did not produce a playable local file"
        }
    }
}
...
enum MediaInputResolver {
    ...
    private static func findCachedAmbisonicFile(cache: URL, videoID: String?) -> URL? {
        ...
        for candidate in candidates
            where candidate.lastPathComponent.lowercased().contains(explicitMarker) {
    ...
    private static func discoverMultichannelFormatID(_ input: String) -> String? {
        ...
        for format in formats {
            ...
            let bitrate = (format["abr"] as? Double)
                ?? (format["tbr"] as? Double)
                ?? -1
    ...
    private static func resolveYouTube(_ input: String) throws -> ResolvedMediaInput {
        ...
        if let ambisonic {
            print("[audio] Ambisonic sidecar: \(ambisonic.path)")
        }
        ...
        print("[youtube] local file: \(path)\(eac ? " (EAC360)" : "")")
        return ResolvedMediaInput(
            url: url,
            youtubeEACHint: eac,
            ambisonicURL: ambisonic,
