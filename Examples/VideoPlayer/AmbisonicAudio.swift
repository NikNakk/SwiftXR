...
enum AmbisonicAudioError: Error, CustomStringConvertible {
    ...
    var description: String {
        switch self {
        case let .ffmpegUnavailable(error):
            return "Could not launch ffmpeg: \(error.localizedDescription)"
        case let .ffmpegFailed(status):
            return "ffmpeg AmbiX conversion exited with status \(status)"
        case let .invalidCache(message):
            return "Invalid AmbiX cache: \(message)"
        case .channelLayoutCreationFailed:
            return "Could not create ACN/SN3D Ambisonic channel layout"
        case .formatCreationFailed:
            return "Could not create four-channel Ambisonic processing format"
        case .psvr2AudioDeviceMissing:
            return "PS VR2 CoreAudio output device not found"
        case .outputAudioUnitMissing:
            return "AVAudioEngine output node has no AudioUnit"
        case let .outputRoutingFailed(status):
            return "Could not route AVAudioEngine to PS VR2 (OSStatus \(status))"
        }
    }
}
...
final class AmbisonicAudio {
    ...
    private let ambisonicFormat: AVAudioFormat

    init(sidecarURL: URL) throws {
        self.sourceURL = sidecarURL
        self.cacheURL = sidecarURL
        ...
        print("[audio] head-tracked AmbiX enabled (ACN/SN3D -> Apple headphone renderer)")
    }
    ...
    func startSynchronized(videoPlayer: AVPlayer, mediaTimeSeconds: Double) throws {
        ...
        let audioStart = AVAudioTime(hostTime: hostTime)
        playerNode.play(at: audioStart)
    ...
    private func routeEngineToPSVR2() throws {
        ...
        print("[audio] routed Ambisonic engine to headset: \(device.name)")
    }

    private static func buildNativeCacheIfNeeded(source: URL, cache: URL) throws {
        if FileManager.default.fileExists(atPath: cache.path) {
            print("[audio] native AmbiX decode cache hit")
            return
        }

        print("[audio] decoding AmbiX sidecar to four-channel float CAF…")
        let process = Process()
