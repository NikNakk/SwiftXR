...
final class VideoLibraryModel: ObservableObject {
    ...
    var closeYouTube: (() -> Void)?
    var youtubeBack: (() -> Void)?
    ...
    func setYouTubeSnapshot(_ image: NSImage?) {
        youtubeSnapshot = image
    }

    func setYouTubeStatus(_ value: String) {
        youtubeStatus = value
    }
    ...
    func page(_ delta: Int) {
        guard !entries.isEmpty else { return }
        selectionIndex = min(
            max(selectionIndex + delta * pageSize, 0),
            entries.count - 1
        )
        ensureSelectionVisible()
    }
    ...
    func openDrives() {
        loadDirectory("/Volumes")
    }

    private func ensureSelectionVisible() {
        if selectionIndex < pageOffset {
            pageOffset = selectionIndex
...
struct VideoPlayerRootView: View {
    ...
    var body: some View {
        Group {
            switch library.mode {
            case .files:
                VideoFileBrowserView(model: library)
            case .youtube:
                VideoYouTubeBrowserView(model: library)
            case .loading:
                VideoLoadingView(model: library)
            case .controls:
                VideoControlsView(model: controls)
            }
        }
            ...
            VStack(spacing: 10) {
                ...
                VStack(spacing: 6) {
                    ForEach(model.visibleEntries) { entry in
                        ...
                        .buttonStyle(.plain)
                        .onHover { hovering in
                            if hovering { model.select(entry) }
                        }
                    }
                ...
                HStack(spacing: 10) {
                    Button { model.page(-1) } label: {
                        Label("Page up", systemImage: "chevron.up")
                    }
                    Button { model.page(1) } label: {
                        Label("Page down", systemImage: "chevron.down")
                    }
                    Button { model.openDrives() } label: {
                        Label("Drives", systemImage: "externaldrive.fill")
                    }
                    Spacer()
                    Button { model.showYouTube() } label: {
