...
/// ScreenCaptureKit -> Core Video -> Metal bridge for the virtual-desktop demo.
///
/// No pixel copy is performed here. ScreenCaptureKit supplies a CVPixelBuffer and
/// CVMetalTextureCache exposes that IOSurface-backed image to the same MTLDevice
/// selected by the OpenXR runtime. The latest frame is retained until a newer
/// one arrives.
final class DesktopCapture: NSObject, SCStreamOutput, SCStreamDelegate, @unchecked Sendable {
    ...
    private let outputQueue = DispatchQueue(
        label: "com.niknakk.swiftxr.virtual-desktop.capture",
        qos: .userInteractive
    )
    ...
    private func makeFrame(
        ...
        texture.label = "SwiftXR captured macOS desktop"
        return (
            cvTexture,
            DesktopCaptureFrame(
                texture: texture,
